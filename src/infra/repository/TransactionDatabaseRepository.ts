  ...
  implements TransactionRepository
{
  ...
  async save(transaction: Transaction): Promise<void> {
    await this.connection.query(
      "insert into example.transaction (code, amount, number_installments, payment_method) values ($1, $2, $3, $4)",
      [
        transaction.code,
        transaction.amount,
    ...
    for (const installment of transaction.installments) {
      await this.connection.query(
        "insert into example.installment (code, number, amount) values ($1, $2, $3)",
  ...
  async get(code: string): Promise<Transaction> {
    const transactionData = await this.connection.one(
      "select * from example.transaction where code = $1",
      [code]
    );
    ...
    const installmentsData = await this.connection.query(
      "select * from example.installment where code = $1",
      [code]
    );
